...
from data.vision import Recognizer, LayoutRecognizer, TableStructureRecognizer, OCR, init_in_out
import argparse
import re
import numpy as np
...
def run_layout_recognition(inputs, mode="layout", threshold=0.5, output_dir="./layout_outputs"):
    """
    运行文档布局识别或表格结构识别（TSR）
    :param inputs: 可以是图像/PDF文件路径，或包含这些文件的目录
    :param mode: 任务模式，"layout"（版面识别）或 "tsr"（表格结构识别）
    :param threshold: 过滤检测的阈值，默认 0.5
    :param output_dir: 结果存放目录
    :return: 解析结果（JSON 格式的布局信息）
    """
    os.makedirs(output_dir, exist_ok=True)
    images, outputs = init_in_out(inputs, output_dir)

    if mode.lower() == "layout":
        labels = LayoutRecognizer.labels
        recognizer = Recognizer(labels, "layout", os.path.join(os.path.dirname(__file__), "../../rag/res/data/"))
    elif mode.lower() == "tsr":
        labels = TableStructureRecognizer.labels
        recognizer = TableStructureRecognizer()
        ocr = OCR()

    layouts = recognizer(images, float(threshold))
    results = []
    ...
    for i, lyt in enumerate(layouts):
        ...
        results.append(lyt_data)

        # 保存带标注的图片
        img = draw_box(images[i], lyt, labels, float(threshold))
