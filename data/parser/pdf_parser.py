...
from data.vision import OCR, Recognizer, LayoutRecognizer, TableStructureRecognizer
...
def extract_tables_manually(pdf_path):
    """ 使用 pdfplumber 提取表格 """
    tables_data = []
    try:
        with pdfplumber.open(pdf_path) as pdf:
            for i, page in enumerate(pdf.pages):
                ...
                if tables:
                    logging.info(f"✅ `pdfplumber` 在第 {i + 1} 页找到 {len(tables)} 个表格")
                    tables_data.append(tables)
                else:
                    logging.info(f"⚠️ `pdfplumber` 在第 {i + 1} 页没有找到表格")
    except Exception as e:
        logging.error(f"❌ `pdfplumber` 解析表格失败: {e}")
    return tables_data
...
class PdfParser:
    ...
    def __init__(self):
        ...
        try:
            ...
            model_dir = snapshot_download(
                ...
                local_dir=os.path.join(get_project_base_directory(), "rag/res/data"),
    ...
    def __call__(self, fnm, need_image=True, zoomin=3, return_html=False):
        self.pdf_path = fnm  # 保存 PDF 路径，供 pdfplumber 使用
        ...
        # **原有 TableStructureRecognizer**
        tables, figures = self._extract_table_figure(need_image, zoomin, return_html, False)

        # **如果 `TableStructureRecognizer` 失败，则使用 `pdfplumber`**
        if not tables:
            logging.warning("⚠️ `TableStructureRecognizer` 解析失败，尝试 `pdfplumber` 解析表格")
            tables = self._extract_tables_manually(fnm)

        return self.__filterout_scraps(deepcopy(self.boxes), zoomin), tables
    ...
    def _extract_table_figure(self, need_image, ZM,
        ...
        # **如果 TableStructureRecognizer 没有检测到表格，尝试 pdfplumber**
        if not tables:
            logging.warning("⚠️ `TableStructureRecognizer` 解析表格失败，尝试使用 `pdfplumber`")
            pdf_tables = extract_tables_manually(self.pdf_path)
            if pdf_tables:
                tables["pdfplumber_extracted"] = pdf_tables
                logging.info(f"✅ `pdfplumber` 解析成功，找到 {len(pdf_tables)} 个表格")
    ...
    def __call__(self, fnm, need_image=True, zoomin=3, return_html=False):
        self.pdf_path = fnm  # 确保 PDF 路径正确赋值
        ...
        if not tables:
            logging.warning("⚠️ `TableStructureRecognizer` 解析失败，尝试 `pdfplumber` 解析表格")
            tables = extract_tables_manually(self.pdf_path)  # 这里的 `self.pdf_path` 现在已正确定义
