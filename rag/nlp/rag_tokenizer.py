...
if __name__ == '__main__':
    ...
    # 可视化评分标签
    def print_test_result(original, tokens, score):
        """带颜色标记的测试结果打印"""
        color_code = "\033[92m" if score > 0.7 else "\033[93m" if score > 0.5 else "\033[91m"
        reset_code = "\033[0m"
        print(f"{color_code}输入：{original}\n分词：{tokens} (置信度：{score:.2f}){reset_code}")
    ...
    test_cases = [
        (
            "根据《XX大学学生管理办法》第一章第三条，学生旷课累计超过10学时需接受纪律处分。",
            ["根据", "《", "XX大学", "学生管理办法", "》", "第一章", "第三条", "，", "学生", "旷课", "累计", "超过", "10",
             "学时", "需", "接受", "纪律处分", "。"]
        ),
        (
            "硕士生学位论文查重率不得超过15%（使用中国知网检测系统）。",
            ["硕士生", "学位论文", "查重率", "不得", "超过", "15%", "（", "使用", "中国知网", "检测系统", "）", "。"]
        ),
        (
            "教务处发〔2023〕6号文件关于修订本科生奖学金评定办法的通知",
            ["教务处", "发", "〔", "2023", "〕", "6号", "文件", "关于", "修订", "本科生", "奖学金", "评定办法", "的",
             "通知"]
        ),
        (
            "学生在校园内使用AI生成作业属于学术不端行为",
            ["学生", "在", "校园内", "使用", "AI生成", "作业", "属于", "学术不端", "行为"]
        ),
        (
            "港澳台学生参照本规定执行",
            ["港澳台学生", "参照", "本规定", "执行"]
    ...
    print("\n\033[1;36m=== 学校政策文件分词测试 ===\033[0m")
    for idx, (text, expected) in enumerate(test_cases, 1):
        # 基础分词
        coarse_tokens = tokenizer.tokenize(text)
        # 细粒度分词
        fine_tokens = tokenizer.fine_grained_tokenize(text)

        # 计算置信度（基础分词与预期匹配率）
        matched = sum(1 for t in coarse_tokens if t in expected)
        score = matched / len(expected) if expected else 0.0

        # 打印带颜色标记的结果
        print(f"\n\033[1;34m用例 {idx}:\033[0m")
        print_test_result(text, coarse_tokens, score)
        print_test_result("细粒度分词", fine_tokens, 1.0)  # 细粒度默认高分

    # 命令行自定义词典测试
    if len(sys.argv) > 2:
        print("\n\033[1;36m=== 自定义词典测试 ===\033[0m")
        tokenizer.DEBUG = False
        tokenizer.loadUserDict(sys.argv[1])
        ...
        with open(sys.argv[2], "r") as f:
            while line := f.readline():
                ...
                if cleaned:
                    print(f"\n原始文本: {cleaned}")
                    print("分词结果:", tokenizer.tokenize(cleaned))
